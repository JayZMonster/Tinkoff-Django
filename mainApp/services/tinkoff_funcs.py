...
def make_order_tick(data_s, token):

    ticker = data_s['ticker']
    operation = data_s['order_action']
    qty = data_s['order_contracts']
    figi_name = None
    ...
    with Client(token) as tinkoff_client:
        ...
        if not figi_name:
            return 'Нет такой акции!'
        try:
            resp = tinkoff_client.users.get_accounts()
            # TODO remove print
            print(resp.accounts)
            acc_id = resp.accounts[0].id
        except:
            return 'Проблемы с получением ID счета!'
